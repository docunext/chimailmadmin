...
<h4>Alias List for <xsl:value-of select="//domains_get_all[domain_id=//_get/domain_id]/domain"/></h4>
<div if="myAliasDiv">
<table id="alias_table" class="tablesorter">
    ...
    <tbody>
    <xsl:for-each select="//alias_get_all">
    <tr>
        <td><xsl:value-of select="alias_address"/></td>
        <td></td>
        <td></td>
